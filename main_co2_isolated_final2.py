...
#         (2.5 * 3600, 3650),  # After 2.5 hrs, should be at least 10 years
...
def run(physics_type: str, case: str, out_dir: str, export_vtk=True, redirect_log=True, platform='cpu', save_ring=True,
    ...
    print(f"RESULT_FOLDER_NAME={out_dir}")
    ...
    def output_to_satV(self, output_directory, threshold=0.05):
        ...
        i_well, j_well = 15 - 1, 15 - 1 #90 - 1, 95 - 1
        ...
        for ith_step in range(0, len(self.idata.sim.time_steps) + 1):
            print(f"\n--- Timestep {ith_step} ---")
            ...
            print(f"Max satV in top layer: {satV_top.max():.4f}")
            active_cells = np.where(satV_top >= threshold)[0]
            print(f"Cells above threshold: {len(active_cells)}")
            ...
            if len(active_cells) > 0:
                sample_idx = active_cells[0]
                print(
                    f"Sample cell: satV={satV_top[sample_idx]:.3f}, dx={x_centers[sample_idx] - x_well:.2f}, dy={y_centers[sample_idx] - y_well:.2f}, direction={directions[sample_idx]}")

            # Compute plume surface area
            plume_cell_count = len(active_cells)
    ...
    def save_directional_plume_to_csv(directional_plume, output_directory, filename='plume_migration.csv'):
        ...
        # Only take directional columns for max calc
        direction_cols = ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW']
        df['MaxDistance'] = df[direction_cols].max(axis=1)
        df['MaxDirection'] = df[direction_cols].idxmax(axis=1)
    ...
    directional_plume = output_to_satV(m, output_directory=out_dir, threshold=0.05)
    ...
    #     # Manual well location (1-based), convert to 0-based
    #     i_well, j_well = 90 - 1, 95 - 1 #105 - 1 , 59 - 1 #14, 14 #90 - 1, 95 - 1 #WATCH OUT, CHANGE!
    #     x_well, y_well = get_cell_center(i_well, j_well, coord, nx)
    ...
    def add_columns_time_data(time_data):
        ...
        for k in list(time_data.keys()):
            if physics_type == 'ccs' and 'well_INJ_molar_rate_CO2_by_sum_perfs' in k:
                time_data[k.replace('well_INJ_molar_rate_CO2_by_sum_perfs', 'V rate (ton/day)')] = time_data[
                                                                                                       k] * molar_mass_co2 / 1000  # kmol/day * kg/kmol / 1000 = ton/day
    ...
    reporting_units = ['components_mass_rates',
                       "components_molar_rates"]  # ["phases_molar_rates","phases_mass_rates","phases_volumetric_rates","components_molar_rates","components_mass_rates"]
    ...
    if export_vtk:
        ...
        output_properties = ['satV', 'satA', 'xCO2', 'yCO2', 'xH2O', 'yH2O', 'pressure',
...
def plot_results(wells, well_is_inj, time_data_list, time_data_report_list,
    ...
    def plot_total_inj_gas_rate_darts_volume(
        ...
        inj_cols = [col for col in acc_df.columns if
                    "V rate (ton/day)" in col]  # "INJ" in col and "V rate (ton/day)" in col]
...
if __name__ == '__main__':
    ...
    #cases_list += ["fault=FM1_cut=CO1_grid=G1_top=TS2_mod=PIX_mult=2"]
    ...
    for physics_type in physics_list:
        for case_idx, case_geom in cases_list:
            for wctrl in well_controls:
                ...
                tag = f"{int(case_idx):03d}" + "_" + "10_BAR_NO_WI_ADJUSTED_MS_FALSE"  # "WI_ADJUSTED_SKIN_14_10BAR_3_YEAR" #"TEST_ORIGINAL_4_bar_FINAL"

                case = case_geom + '_' + wctrl
                folder_name = 'results_' + physics_type + '_' + case + '_' + tag
